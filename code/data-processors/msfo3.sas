...
proc sql;
	...
	inner join prcs.ozone_site_zip_bz6 b
...
/*change . to 0*/
data prcs.master_ndi_ozone; set master_cuz;
...
proc export	data=prcs.master_ndi_ozone
	outfile='/scratch/fatemehkp/projects/Ozone/data/processed/master_ndi_ozone.csv'
	dbms=csv
	replace;
run;

proc sort data= prcs.master_ndi_ozone nodupkey out= prcs.site_ozone_ndi (keep= Site_ID);
	by Site_ID; 
run;

proc export	data=prcs.site_ozone_ndi
	outfile='/scratch/fatemehkp/projects/Ozone/data/processed/site_ozone_ndi.csv'
	dbms=csv
	replace;
run;
