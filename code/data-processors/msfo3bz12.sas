...
proc sql;
	create table raw.enrollee_ozone_bz12 as
	select *
	from cms.enrollee65_ndi_0008_clean a 
	inner join prcs.ozone_site_zip_bz12 b
	on a.zip_code=b.zip_code and a.year=b.year
	where sex ne 'U';
quit;

data enrollee_ozone; 
	set raw.enrollee_ozone_bz12;
...
/*change . to 0*/
data prcs.master_ndi_ozone_bz12; set master_cuz;
...
proc export	data=prcs.master_ndi_ozone_bz12
	outfile='/scratch/fatemehkp/projects/Ozone/data/processed/master_ndi_ozone_bz12.csv'
	dbms=csv
	replace;
run;
